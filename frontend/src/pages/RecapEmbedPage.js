...
const RecapEmbedPage = () => {
  ...
  const currentIndex = recaps.findIndex((r) => r.id === recapId);
      ...
      <div className="flex-1 flex flex-col">
        <div className="flex justify-between items-center px-6 py-4 bg-white shadow">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded ${currentIndex === 0 ? "bg-gray-300" : "bg-blue-500 text-white hover:bg-blue-600"}`}
          >
          ...
          <button
            ...
            onClick={() => navigate(`/recaps/${recaps[currentIndex + 1].id}`)}
            className={`flex items-center gap-2 px-4 py-2 rounded ${currentIndex === recaps.length - 1 ? "bg-gray-300" : "bg-blue-500 text-white hover:bg-blue-600"}`}
          >
            Next <FaArrowRight />
          </button>
        </div>
